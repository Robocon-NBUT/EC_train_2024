...
/* USER CODE BEGIN 1 */
	...
	{ 
		...
		for (int i = 0; i < 4; i++) {
        PID_struct_init(&pid_motor[i], POSITION_PID, 16384, 16384, 5.5f, 0.1f, 0.0f);
    }
...
set_speed[i] = pid_calc(&pid_motor[i], (float)motor_chassis[i].speed_rpm, 5000 );
CAN_Send( &hfdcan1,set_speed[0], 0,0, 0);
//mypid.out = pid_calc(&mypid,(float)motor_chassis[0].speed_rpm, 5000);
//CAN_Send(&hfdcan1,mypid.out,400,400,400);
 break; 
 }
 default: 
 {
