...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	...
	float set_speed_temp;			   //加减速时的临时设定鿟�?
	int16_t delta;					   //设定速度与实际鿟度的差�??
	int16_t max_speed_change = 1000;   //电机单次朿大变化鿟度，加减鿟�?
									   // 500经测试差不多是最大加速区间，即从零打到最大鿟度不异常的朿大忿
	static float set_speed[4];
  ...
  /* USER CODE BEGIN 2 */
	OLED_Init();
	CAN_Open(&hfdcan1);
  CAN_Send( &hfdcan1,100, 4000, 4000, 4000 );
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		//CAN_Send( &hfdcan1,1, 4000, 4000, 4000 );
