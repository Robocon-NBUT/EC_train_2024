...
#define DEADZONE 100.0f 
#define MAX_CONTROL_SIGNAL 80.0f  
#define MIN_CONTROL_SIGNAL -80.0f 
...
float PIDCalc(PID *pp, float NextPoint) {
    ...
    float output = (pp->Proportion * Error  
                     + pp->Integral * pp->SumError  
                     + pp->Derivative * dError); 
		
		if (output > MAX_CONTROL_SIGNAL) {
        output = MAX_CONTROL_SIGNAL;
    } 
		else if (output < MIN_CONTROL_SIGNAL) {
        output = MIN_CONTROL_SIGNAL;
    }
...
float CascadePIDControl(int vSetPosition, int vSetSpeed, float encoderPosition, float encoderSpeed) {
    static PID PositionPID = {0, 0.5, 0.01, 0.05, 0, 0, 0}; 
    static PID SpeedPID = {0, 0.5, 0.01, 0.001, 0, 0, 0};  

    PositionPID.SetPoint = vSetPosition; 
    if (fabs(vSetPosition - encoderPosition) < DEADZONE) 
		{
        vSetSpeed = 0.0f;  
    } 
		else 
		{
        vSetSpeed = PIDCalc(&PositionPID, encoderPosition);  
    }
